<?php
class ModelSaleNovinPayamak extends Model {
	public function getNovinPayamakReports($data) {
		
		$sql = "SELECT * FROM " . DB_PREFIX . "novinpayamak_report";
		
		$implode = array();
		
		if (isset($data['filter_id']) && !is_null($data['filter_id'])) {
			$implode[] = "novinpayamak_report_id = '" . $this->db->escape($data['filter_id']) . "'";
		}
		
		if (isset($data['filter_source']) && !is_null($data['filter_source'])) {
			$implode[] = "novinpayamak_source LIKE '%" . $this->db->escape($data['filter_source']) . "%'";
		}
		
		if (isset($data['filter_destination']) && !is_null($data['filter_destination'])) {
			$implode[] = "novinpayamak_destination LIKE '%" . $this->db->escape($data['filter_destination']) . "%'";
		}
		
		if (isset($data['filter_message']) && !is_null($data['filter_message'])) {
			$implode[] = "novinpayamak_message LIKE '%" . $this->db->escape($data['filter_message']) . "%'";
		}
		
		if (isset($data['filter_message_type']) && !is_null($data['filter_message_type'])) {
			$implode[] = "novinpayamak_message_type = '" . $this->db->escape($data['filter_message_type']) . "'";
		}
		
		if (isset($data['filter_server_status']) && !is_null($data['filter_server_status'])) {
			$implode[] = "novinpayamak_server_status LIKE '%" . $this->db->escape($data['filter_server_status']) . "%'";
		}
		
		if (isset($data['filter_start_date']) && !is_null($data['filter_start_date'])) {
			$implode[] = "novinpayamak_sent_on >= '" . $this->db->escape($data['filter_start_date']) . "'";
		}
		
		if (isset($data['filter_end_date']) && !is_null($data['filter_end_date'])) {
			$implode[] = "novinpayamak_sent_on <= '" . $this->db->escape($data['filter_end_date']) . "'";
		}
		...
		$sort_data = array(
			'novinpayamak_report_id',
			'novinpayamak_source',
			'novinpayamak_destination',
			'novinpayamak_message',
			'novinpayamak_message_type',
			'novinpayamak_server_status',
			'novinpayamak_sent_on'
		);	
			
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			$sql .= " ORDER BY " . $data['sort'];	
		} else {
			$sql .= " ORDER BY novinpayamak_report_id";	
		}
